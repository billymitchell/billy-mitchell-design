...
export default () => (
  <StaticQuery
    query={graphql`
    {
      allAirtable(filter: {table: {eq: "Project"}, data: {Creative_Discipline: {eq: "Video Production"}, Published: {eq: true}, Featured: {ne: true}}}, sort: {order: DESC, fields: data___End_Date}) {
        nodes {
          recordId
          data {
            Project_Title
