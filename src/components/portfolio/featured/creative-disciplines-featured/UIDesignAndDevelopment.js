...
export default () => (
  <StaticQuery
    query={graphql`
    {
      allAirtable(filter: {table: {eq: "Project"}, data: {Creative_Discipline: {eq: "UI Design & Development"}, Published: {eq: true},Featured: {eq: true}}}, sort: {order: DESC, fields: data___End_Date}) {
        nodes {
          recordId
          data {
            Project_Title
